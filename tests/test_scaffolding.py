...
def test_run_init_creates_scaffolding(tmp_path):
    ...
    sample_claim = claims_data[0]
    assert sample_claim["claim_id"] == "CLAIM-VT-9999"
    assert "__field_hints__" in sample_claim
    assert "claim_id" in sample_claim["__field_hints__"]
    assert "申索ID，必须符合正则格式" in sample_claim["__field_hints__"]["claim_id"]
    ...
    assert "__field_hints__" in tasks_data
    assert "tasks" in tasks_data["__field_hints__"]
    assert isinstance(tasks_data["tasks"], list)
    assert len(tasks_data["tasks"]) == 1
    sample_task = tasks_data["tasks"][0]
    assert sample_task["task_id"] == "TASK-VT-9999"
    assert "__field_hints__" in sample_task
    assert "task_id" in sample_task["__field_hints__"]
    assert "任务ID，必须符合正则格式" in sample_task["__field_hints__"]["task_id"]

    # Verify config.json content
    with config_path.open("r", encoding="utf-8") as f:
        config_data = json.load(f)
    assert config_data["prd_path"] == "docs/prd.md"
    ...
    # Run init again
    exit_code_again = run_init(tmp_path)
