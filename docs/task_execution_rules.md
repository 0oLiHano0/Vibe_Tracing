...
> 2. 在 `.vibetracing/claims/CLAIM-*.json` 中申报一条状态为 `blocked` 的 Claim（每个 Claim 独立文件）。
...
### B. 路径与引用的完整性
当生成 `.vibetracing/claims/CLAIM-*.json` 时（每个 Claim 独立文件），所有文件和代码引用（`code_refs` 和 `test_refs`）必须指向代码库中真实存在的文件。

> [!NOTE]
> **已废弃字段**：`evidence_refs` 字段已废弃。Claim 已降级为指针+工作日志，不再承担自证证据链的职责。证据由 `evidences/test_results.json` 和 `evidences/coverage_reports.json` 独立管理。
