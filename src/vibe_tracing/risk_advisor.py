...
class RiskAdvisor:
    ...
    def generate_risks(
        ...
        for orig_risk in claim_risks:
            ...
            # Match patterns to generate detailed advice
            if "self-referential" in desc or "only self-referential" in desc:
                ...
                "non-existent evidence" in desc
                or "references non-existent evidence" in desc
            ):
                impact = "证据链中断，导致声称的完成状态无法被客观核查。"
                action = "核对 `evidence_index.json` 或工具报告输出，确保被引用的证据项已正确生成并包含在索引中。"
            elif "modified after" in desc or "claim timestamp" in desc:
                impact = "文件在此次 Claim 签发后发生了修改，声称的覆盖状态已失效，属于推测性风险。"
                action = "重新验证该任务以生成包含最新时间戳的 Agent Claim 记录。"
                r["confidence"] = "low_confidence"
                r["type"] = "suggestion"
            elif "non-existent task" in desc:
                impact = (
                    "Claim 关联到了不存在的任务，可能由于任务 ID 写错或任务列表未同步。"
                )
                action = "检查该 Claim 的 `related_task` 字段是否与 `task_list.json` 中的任务 ID 一致。"
            elif "non-existent" in desc and ("code" in desc or "test" in desc):
                impact = (
                    "Claim 中引用的物理文件在工作区中不存在，无法建立有效的物理追溯链。"
                )
                action = (
                    "核实文件路径，检查是否存在拼写错误、文件被重命名或未提交的情况。"
                )
            elif "no test coverage" in desc:
                impact = "关联的验收标准没有通过测试或测试执行失败，导致无法提供客观的合格证明。"
                action = "针对该验收标准补充测试用例，并在测试函数的 docstring 中声明 `covers: <ac_id>`。"
            elif "failed tests" in desc or "has failed tests" in desc:
        ...
        for gap in gaps:
            ...
            if item_type == "requirement":
                enriched_risks.append(
                    {
                        "risk_id": f"RISK-VT-{next_counter:03d}",
                ...
                enriched_risks.append(
                    {
                        ...
                        "evidence_ids": [v.get("evidence_id", "EVIDENCE-VT-999")],
                    }
                )
                next_counter += 1
